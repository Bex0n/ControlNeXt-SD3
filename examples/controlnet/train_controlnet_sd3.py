...
from ...models.controlnets.controlnet_sd3 import SD3ControlNeXtEncoder
from ...models.transformers.transformer_sd3 import SD3Transformer2DModel
from ...pipelines.controlnet_sd3.pipeline_stable_diffusion_3_controlnet import \
    StableDiffusion3ControlNeXtPipeline
...
def log_validation(controlnet, args, accelerator, weight_dtype, step, is_final_validation=False):
    ...
    if not is_final_validation:
        ...
        controlnet = SD3ControlNeXtEncoder.from_pretrained(args.output_dir, torch_dtype=weight_dtype)

    pipeline = StableDiffusion3ControlNeXtPipeline.from_pretrained(
...
def main(args):
    ...
    # <<< START CONTROLNEXT INJECTION BLOCK >>>
    if args.controlnet_model_name_or_path:
        logger.info("Loading existing ControlNeXt encoder weights")
        controlnet = SD3ControlNeXtEncoder.from_pretrained(args.controlnet_model_name_or_path)
    else:
        logger.info("Initializing ControlNeXt encoder weights from transformer")
        controlnet = SD3ControlNeXtEncoder.from_transformer(
            transformer,
            num_layers=1,
            num_extra_conditioning_channels=args.num_extra_conditioning_channels,
        )
    # <<<< END CONTROLNEXT INJECTION BLOCK >>>>
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                # <<< START CONTROLNEXT INJECTION BLOCK >>>
                controlnext_embedding = controlnet(
                ...
                model_pred = transformer(
                    ...
                    encoder_hidden_states=prompt_embeds,
                    pooled_projections=pooled_prompt_embeds,
                    controlnext_embedding=controlnext_embedding,
